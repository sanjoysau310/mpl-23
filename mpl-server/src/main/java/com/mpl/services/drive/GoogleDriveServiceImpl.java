...
public class GoogleDriveServiceImpl implements GoogleDriveService {
	...
	public Credential getCredentials(NetHttpTransport HTTP_TRANSPORT) throws IOException {
		...
		GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(HTTP_TRANSPORT, JSON_FACTORY,
				secrets, scopes).setDataStoreFactory(new FileDataStoreFactory(new java.io.File(TOKENS_DIRECTORY_PATH_PROD)))
		...
		//LocalServerReceiver receiver1 = new LocalServerReceiver.Builder().setHost(ip.toString()).setPort(8081).build();
